...
> Cloudflare Pages + D1 + R2 + KV + Workers による完全無料デプロイ
...
5. [Cloudflareリソース作成（D1 / R2 / KV）](#5-cloudflareリソース作成d1--r2--kv)
...
| Secret名 | 用途 | 設定先 |
|---|---|---|
| `ADMIN_PASSWORD` | 管理画面ログイン | Pages & Cronワーカー両方 |
...
> ⚠️ **重要**: `wrangler pages deploy` だけではバインディング（D1/R2/KV）が自動設定されません。Cloudflare ダッシュボードで手動追加が必要です。
...
|---|---|
| Variable name | `R2` |
| R2 bucket | `ken-gei-prelude-flyers` |
...
**原因**: D1/R2/KV のバインディングが Pages に設定されていない
...
**解決**: [手順10](#10-pages-プロジェクトにバインディング追加) で R2 バインディングを追加
...
| R2 | 10GB ストレージ, 10M リクエスト/月 | チラシ画像 |
| KV | 100K 読取り/日, 1K 書込み/日 | キャッシュ |
